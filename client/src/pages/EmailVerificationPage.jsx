...
function EmailVerificationPage() {
  ...
  const { error, isLoading, verifyEmail } = useStore();
  ...
  const handleChange = (index, value) => {
    ...
    if (value.length > 1) {
      ...
      const focusIdx = lastFilledIdx < 5 ? lastFilledIdx + 1 : 5;
      inputRefs.current[focusIdx].focus();
    } else {
      newCode[index] = value;
      setCode(newCode);

      //Move focus to next input field if value in entered
      if (value && index < 5) {
      ...
      >
        ...
        <form className="space-y-6">
          ...
          <Motion.button
            ...
            className="w-full bg-linear-to-r from-green-500 to-emerald-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:to-emerald-700 hover:from-green-600 focus:outline-none focus:ring-2 focus:ring-green-500/50 disabled:opacity-50"
          >
            {isLoading ? "Verifying..." : "VerifyEmail"}
          </Motion.button>
        </form>
      </Motion.div>
    </div>
